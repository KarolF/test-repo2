...
//			box.shapes.add(new Polygon(Polygon.box(16, 32)));
			box.shapes.add(new Circle(16));
			box.position.setxy((w / 2)-64*2+Math.floor(i/20)*32, ((h - 50) - 32 * (i%20 + 0.5)));
			box.space = space;
			boxes.push(box)
		}
	...
	private function enterFrameHandler(ev:Event):void {
		...
		space.step(3 / stage.frameRate);
//		space.step(1 / stage.frameRate);
//		space.step(1 / stage.frameRate);

		for each (var body:Body in boxes) {
			if(body.position.y>800)
				body.position.setxy(w/2-32+Math.floor(boxes.indexOf(body)/20)*16,0);
		}
